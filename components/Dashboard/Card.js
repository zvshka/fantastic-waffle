const Tag = ({text}) => {
    return <span className="m-1 px-2 py-1 rounded bg-secondary">
        #{text}
    </span>
}
...
export const Card = ({course}) => {
    return (
        <div className="overflow-hidden shadow-lg transition
            ...
            hover:shadow-2xl rounded-lg h-90 w-70 md:w-80 cursor-pointer m-auto">
            <a href="#" className="w-full block h-full">
                <div className="bg-white w-full p-4">
                    ...
                    items-center py-3 border-b-2 text-xs text-white font-medium">
                        {course.tags && course.tags.map((t, i) => <Tag text={t} key={i}/>)}
                    </div>
